...
def load_subs_into_one_matrix():

    varr = {}
    
    varr['a_path'] = "C:\\Users\\jamilah\\Documents\\Github_analysis_PROJECTS\\Time_series_analysis\\Motor_classification\\Motor_classification\\a_data_standardization"  # Windows
    ...
    varr['main_path'] = "C:\\Users\\jamilah\\Documents\\Github_analysis_PROJECTS\\Time_series_analysis\\Motor_classification\\Motor_classification"  # Windows
    ...
    for exp in range(2):  # 0=rotation, 1=translation
                    ...
                    for dirC in range(2):
                        for mj in range(3):
                            for strict in range(4):
                                ...
                                file_name = "%s\\%s\\s%d_orax%d_yr%d_dirC%d_marg%d_st%d_outSIG.pkl" % (varr['a_path'], filename, s, orax, yr, dirC, mj, strict)
        ...
        fig.write_image("%s\\%s.png" % (varr['a_path'], save_filename0))
        ...
        fig.write_image("%s\\%s.png" % (varr['a_path'], save_filename1))
        ...
        fig.write_image("%s\\%s.png" % (varr['a_path'], save_filename2))
