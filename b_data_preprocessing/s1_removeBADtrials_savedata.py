...
def s1_removeBADtrials_savedata(varr):
    ...
    for exp in range(2):  # 0=rotation, 1=translation
        ...
        fig.show()
        naame1 = '%s\\trial_reject_stats_%s_%s_%s' % (varr['main_path2'], varr['which_exp'], strictness, strictness1)
        fig.write_image("%s.png" % (naame1))
        # ------------------------------
        
        # ------------------------------
        # Save data matrices to file per experiment
        file_name = "%s\\%sdat.pkl" % (varr['main_path2'], varr['which_exp'])
        ...
        # ------------------------------
        # Save data matrices to file per experiment
        file_name = "%s\\exp_reject_cat_vec_%s.pkl" % (varr['main_path2'], varr['which_exp'])
        open_file = open(file_name, "wb")
    ...
    # elif exp == 1:
        ...
        # 40 = cut_trial_hor_short
        # 50 = cut_trial_standard
    file_name = "%s\\exp_sub_statistic.pkl" % (varr['main_path2'])
    open_file = open(file_name, "wb")
