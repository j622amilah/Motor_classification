...
from numderiv import numderiv
from make_a_properlist import make_a_properlist
from detect_sig_change_wrt_baseline import detect_sig_change_wrt_baseline


def process_index_for_UD_trials(s, UD_trials2, outSIG, marg, starttrial_index, stoptrial_index, speed_stim_sign, speed_stim_mag, plotORnot, plotORnot_derv):
    
    # A UD trial consist of [initialization mvt + trial mvt + reinitialization mvt] events.
    ...
    for UD_tr in UD_trials2:
        ...
        # ------------------------------
        LR_vals = np.round(outSIG[UD_tr][:, 0])
        #UD_vals = np.round(sig)
        
        baselineLR = mode(LR_vals)
        countLR = 0
        for ii in range(len(LR_vals)):
            if LR_vals[ii] == baselineLR:
                countLR = countLR + 1

        #baselineUD = mode(UD_vals)
        #countUD = 0
        #for ii in range(len(UD_vals)):
        #    if UD_vals[ii] == baselineUD:
        #        countUD = countUD + 1
        #
        #print('baselineLR : ' + str(baselineLR))
        #print('baselineUD : ' + str(baselineUD))
        # ------------------------------
        ...
        # Search along the signal UD from first point, to see when I hit the zone around the baseline; 
        # to find the index that corresponds to the baseline
        dpOFsig_in_zone, indexOFsig_in_zone, dp_sign_not_in_zone, indexOFsig_not_in_zone = detect_sig_change_wrt_baseline(outSIG[UD_tr][:, 2], baseline, marg)
        ...
        # ------------------------------------------
        #starttrial_index[UD_tr] = value_st[0]
        #starttrial_index[UD_tr:UD_tr+1] = int(value_st[0])
        starttrial_index[UD_tr] = int(value_st[0])
        print('starttrial_index[UD_tr:UD_tr+1] : ' + str(starttrial_index[UD_tr:UD_tr+1]))
        # ------------------------------------------
        
        
        # Now, let's determine the stopping x-axis index value of the UD trial.  
        ...
        sig = outSIG[UD_tr][int(starttrial_index[UD_tr:UD_tr+1]):int(stoptrial_index_derv), 2] - outSIG[UD_tr][int(starttrial_index[UD_tr:UD_tr+1]), 2]
        Asig = abs(sig)
        Lsig = len(sig)
        ...
        stoptrial_index_max = starttrial_index[UD_tr:UD_tr+1] + ivalUD
        ...
        # ------------------------------------------
        stoptrial_index_zoneofmax = starttrial_index[UD_tr:UD_tr+1] + ind3[-1]
        #print('stoptrial_index_zoneofmax : ' + str(stoptrial_index_zoneofmax))
        # ------------------------------------------
        
        # --------------------------------------------
        # Check to confirm that new stopPT (stoptrial_index) is better than the original stopPT (stoptrial_index_derv)
        # Each trial needs to at least be 200 data points long (due to EXP timing), we check the length from start pt 
        # to the new stopPT.  If the length is equal to or greater than 200, we use new stopPT.  But, if the length is shorter
        # we use the stop pt obtained from the last signal derivative peak.
        ...
        if len(value_st) == 1:
            ...
            stoptrial_index[UD_tr] = value_end[-1]  # the point where the signal passes out of the baseline zone
            speed_stim_sign[UD_tr] = 0
            speed_stim_mag[UD_tr] = 0
        else:
            # Way 1:
            ...
            #if stoptrial_index_derv > len(outSIG[UD_tr][:,2])-50:
            #    stoptrial_index[UD_tr] = stoptrial_index_zoneofmax
            #else:
            #    # Risks to have a small initial portion of reinitialization
            #    stoptrial_index[UD_tr] = stoptrial_index_derv
            
            # Way 2:
            # Organize all the points in assending order
            pts = starttrial_index[UD_tr][0], value_st, value_end, stoptrial_index_derv, stoptrial_index_max[0], stoptrial_index_zoneofmax[0]
            pts = make_a_properlist(pts)
            pts.sort()
            print('pts : ' + str(pts))
            print('value_end[-1] : ' + str(value_end[-1]))
            
            # Most logic : choose the point before value_end[-1](the return to UD start) 
            keeper = []            
            for ii in range(len(pts)):
                if value_end[-1] > pts[ii]:
                    keeper = keeper + [pts[ii]]
            print('keeper : ' + str(keeper))
            stoptrial_index[UD_tr] = keeper[-1]
        ...
        vec = outSIG[UD_tr][int(starttrial_index[UD_tr][0]):int(stoptrial_index[UD_tr][0]), 2]
        baseline = vec[0]
        marg = 10
        ...
        if len(indexOFsig_in_zone) == len(vec):
            ...
            speed_stim_sign[UD_tr] = np.sign(diff)
        # --------------------------------------------
        ...
        speed_stim_mag[UD_tr] = max(abs(vec))
        # --------------------------------------------

        # --------------------------------------------
        if plotORnot == 1:
            marksize = 15
            L_Osig = len(outSIG[UD_tr][:,2])
            Osig = abs(outSIG[UD_tr][:,2])
            ...
            dp1 = np.array(range(int(starttrial_index[UD_tr][0]), int(stoptrial_index[UD_tr][0])))
            fig.add_trace(go.Scatter(x=dp1, y=outSIG[UD_tr][int(starttrial_index[UD_tr][0]):int(stoptrial_index[UD_tr][0]), 2], name='final signal', line = dict(color='yellow', width=2, dash='dash'), showlegend=True))

            # 0) Start point
            st_pt = starttrial_index[UD_tr][0]*np.ones((2)) # must double the point : can not plot a singal point
            st_pt = [int(x) for x in st_pt] # convert to integer
            print('starttrial_index[UD_tr][0] : ' + str(st_pt))
            ...
            # 3) Max baseline shifted zero abs signal stop point
            sp_i_m = stoptrial_index_max[0]*np.ones((2)) # must double the point : can not plot a singal point
            sp_i_m = [int(x) for x in sp_i_m] # convert to integer
            print('stoptrial_index_max[0]: ' + str(sp_i_m))
            ...
            sp_i_zofm = stoptrial_index_zoneofmax[0]*np.ones((2)) # must double the point : can not plot a singal point
            sp_i_zofm = [int(x) for x in sp_i_zofm] # convert to integer
            print('stoptrial_index_zoneofmax[0]: ' + str(sp_i_zofm))
            ...
            for q in range(len(value_st)):
                ...
                fig.add_trace(go.Scatter(x=vs, y=outSIG[UD_tr][vs, 2], name='st derv peaks', mode='markers', marker=dict(color='cyan', size=5, line=dict(color='cyan', width=0)), showlegend=True))
            ...
            for q in range(len(value_end)):
                ...
                fig.add_trace(go.Scatter(x=ve, y=outSIG[UD_tr][ve, 2], name='end derv peaks', mode='markers', marker=dict(color='orange', size=5, line=dict(color='orange', width=0)), showlegend=True))

            title_str = 'sub: %d, trial: %d, stoppt derv: %d, stoppt max: %d, stoppt zone: %d, value end: %d, sign=%d, mag=%d' % (s, UD_tr, stoptrial_index_derv, stoptrial_index_max, stoptrial_index_zoneofmax, value_end[-1], speed_stim_sign[UD_tr], speed_stim_mag[UD_tr])
